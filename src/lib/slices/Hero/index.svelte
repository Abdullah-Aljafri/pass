...
<Bounded data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
	...
	<h1 class="hero__heading max-w-3xl mx-auto text-balance text-5xl font-medium md:text-7xl opacity-0">
	...
	<ButtonLink class=" hero__button mt-8 opacity-0" field={slice.primary.button_link}>{slice.primary.button_label}</ButtonLink>
		
	{/if}
	{#if slice.primary.image}
		<div class=" hero__image glass-container mt-16 w-fit opacity-0">
			<div class="hero__glow hero__glow--one opacity-0 absolute left-1/3 
			top-0 -z-10 h-2/3 w-2/3 bg-violet-700/50
			mix-blend-screen blur-3xl md:blur-[120px] filter"/>
			
			<div class="hero__glow hero__glow--two absolute left-0 
			top-1/3 -z-10 h-2/3 w-2/3 bg-orange-600/50
			mix-blend-screen blur-3xl md:blur-[120px] filter opacity-0"/>
